...
class DbError  { readonly _tag = "DbError"  as const }

interface UserRepo { find(id: string): Eff<string, Throws<NotFound>> }
interface Logger   { info(msg: string): Eff<void, never> }
...
const ok3 = run(
  provide(UserRepo.get, UserRepo, { find: (id) => succeed(`user-${id}`) })
)
...
// @ts-expect-error
const err3 = run(
  UserRepo.get.flatMap(repo => repo.find("1"))
)
...
const err4 = run(
  UserRepo.get.flatMap(repo =>
    Logger.get.flatMap(log =>
      log.info("hello").flatMap(() => repo.find("1"))
    )
  )
)
