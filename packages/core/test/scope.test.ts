...
import {
  ...
  acquireRelease, scoped, ensuring,
  fork, join, all,
  run, runSync,
  Ref,
} from "../src"
...
describe("acquireRelease + scoped", () => {
  test("release runs on success", async () => {
    ...
    const managed = acquireRelease(
      sync(() => { log.push("acquire"); return "resource" }),
      (r) => sync(() => { log.push(`release:${r}`) }),
    )

    const program = scoped(
      managed.map(r => `used:${r}`)
    )
  ...
  test("multiple resources release in reverse order", async () => {
    ...
    const res = (name: string) => acquireRelease(
      sync(() => { log.push(`acquire:${name}`); return name }),
      (r) => sync(() => { log.push(`release:${r}`) }),
    )
  ...
  test("resource with async acquire", async () => {
    ...
    const managed = acquireRelease(
      sleep(10).flatMap(() => sync(() => { log.push("acquired"); return "db" })),
      (r) => sync(() => { log.push(`released:${r}`) }),
    )
...
describe("scheduler interleaving", () => {
  test("fibers interleave on op budget", async () => {
    ...
    const longChain = (name: string, n: number) => {
      let eff = sync(() => { log.push(`${name}:start`) })
      for (let i = 0; i < n; i++) {
        eff = eff.flatMap(() => succeed(undefined))
      }
      return eff.flatMap(() => sync(() => { log.push(`${name}:end`) }))
    }

    // 3000 ops each — should exceed 2048 budget and yield
    const program = fork(longChain("A", 3000))
      .flatMap(fiberA =>
        fork(longChain("B", 3000)).flatMap(fiberB =>
          join(fiberA).flatMap(() => join(fiberB))
        )
      )
  ...
  test("structured concurrency — children cancel on parent complete", async () => {
    ...
    const child = sleep(100).flatMap(() => sync(() => { childRan = true }))
