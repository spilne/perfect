...
describe("Gen — primitive generators", () => {
  ...
  test("flatMap chains generators", () => {
    const g = Gen.flatMap(Gen.int(1, 5), (n) =>
      Gen.array(Gen.constant(n), { length: n }),
    )
...
describe("forAll", () => {
  ...
  test("fails with the counterexample when the property breaks", async () => {
    ...
    const program = provide(
      (forAll(Gen.int(0, 1000), 200, (n) => n < 500) as any).catch(
        (f: PropertyFailure<number>) => {
          failure = f
  ...
  test("forAll is deterministic under TestRandom", async () => {
    const program = (seed: number) =>
      provide(
        (forAll(Gen.int(0, 1000), 5, (n) => n < 200) as any).catch(
          (f: PropertyFailure<number>) => succeed(f.value),
        ),
        Random,
        new TestRandom(seed),
      )
