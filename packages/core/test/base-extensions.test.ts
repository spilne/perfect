...
import {
  succeed, fail, die, sync, async, sleep,
  fork, forkDaemon, awaitFiber, interrupt,
  uninterruptible, interruptible, yieldNow,
  race, raceEither, raceAll, timeoutFail, timeoutOption,
  onExit, ensuring,
  run, runSync,
  Cause, Exit,
} from "../src"
...
describe("Exit", () => {
  test("Exit.succeed / Exit.failure roundtrip via match", () => {
    ...
    expect(Exit.match(s, (a) => `ok:${a}`, () => "err")).toBe("ok:1")
    expect(Exit.match(f, () => "ok", (c) => `err:${Cause.squash(c)}`)).toBe("err:bad")
  })
...
describe("onExit", () => {
  test("runs handler with Success exit", async () => {
    ...
    const eff = onExit(succeed(7), (exit) => sync(() => { seen = exit }))
  ...
  test("runs handler with Failure exit and propagates the failure", async () => {
    let seen: Exit | null = null
    const eff = onExit(fail("nope"), (exit) => sync(() => { seen = exit }))
...
describe("Interruption masks", () => {
  test("uninterruptible blocks the interrupt until the block finishes", async () => {
    ...
    const work = uninterruptible(
      sleep(20).flatMap(() => sync(() => { ran++; return "done" })),
    )
  ...
  test("interruptible inside uninterruptible allows interrupt again", async () => {
    let ran = 0
    const inner = interruptible(sleep(50).flatMap(() => sync(() => { ran++ })))
    const work = uninterruptible(sync(() => { ran++ }).flatMap(() => inner))
    const eff = fork(work).flatMap((f) =>
      sleep(5).flatMap(() => interrupt(f)).flatMap(() => awaitFiber(f)),
    )
    const exit = await run(eff as any) as Exit
  ...
  test("ensuring finalizer still runs when interrupted", async () => {
    let finalized = false
    const work = ensuring(sleep(100), sync(() => { finalized = true }))
    const eff = fork(work).flatMap((f) =>
      sleep(5).flatMap(() => interrupt(f)).flatMap(() => awaitFiber(f)),
    )
...
describe("forkDaemon", () => {
  test("daemon outlives parent", async () => {
    let ticks = 0
    const daemon = sleep(30).flatMap(() => sync(() => { ticks++ }))
