...
import {
  succeed, fail, sync, async, tryPromise,
  all, service, provide,
  run, runSync,
...
describe("combinators", () => {
  ...
  test("flatMap", () => {
    const result = runSync(
      succeed(5).flatMap(x => succeed(x + 1))
    )
  ...
  test("tap", () => {
    let sideEffect = 0
    const result = runSync(
      succeed(42).tap(x => sync(() => { sideEffect = x }))
    )
    expect(result).toBe(42)
...
describe("error handling", () => {
  ...
  test("catch does not intercept Die", async () => {
    const eff = sync(() => { throw new Error("die") })
  ...
  test("catchTag narrows error type", () => {
    ...
    const eff: Eff<string, Throws<NotFound | Forbidden>> =
      fail(new NotFound()) as any
  ...
  test("catchTag passes through unmatched errors", async () => {
    ...
    const eff = (fail(new Forbidden()) as Eff<string, Throws<NotFound | Forbidden>>)
      .catchTag("NotFound", () => succeed("nope"))
...
describe("services", () => {
  test("provide and get", () => {
    ...
    let logged = ""
    const program = Logger.get.flatMap(
      logger => sync(() => { logger.log("hello"); return "done" })
    )

    const result = runSync(
      provide(program, Logger, { log: (msg) => { logged = msg } })
    )
