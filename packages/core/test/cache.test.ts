...
import { succeed, fail, sync, cached, cachedBy, provide, run, Clock, TestClock } from "../src";

describe("cached", () => {
  test("runs the source once, replays thereafter", async () => {
    let runs = 0;
    const getConfig = cached(
      sync(() => {
        runs++;
        return Math.random();
      }),
  ...
  test("each cached() call produces an independent memo", async () => {
    ...
    const program = (a as any).flatMap((va: number) =>
      (b as any).map((vb: number) => ({ va, vb })),
    );
...
describe("cachedBy", () => {
  test("same key replays; different keys run separately", async () => {
    let runs = 0;
    const cache = cachedBy((id: string) =>
      sync(() => {
        runs++;
        return `value-for-${id}`;
      }),
  ...
  test("invalidate drops a specific key", async () => {
    ...
    const program = (cache.get("a") as any).flatMap((a1: number) =>
      cache
        .invalidate("a")
        .flatMap(() => (cache.get("a") as any).map((a2: number) => ({ a1, a2 }))),
    );
    ...
    const program = cache
      ...
      .flatMap((empty: boolean) =>
        (cache.get("x") as any).flatMap(() =>
          cache
            .has("x")
            .flatMap((filled: boolean) =>
              cache
                .invalidate("x")
                .flatMap(() => cache.has("x").map((gone: boolean) => ({ empty, filled, gone }))),
            ),
        ),
      );
