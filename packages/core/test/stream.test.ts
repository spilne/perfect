...
describe("Chunk", () => {
  ...
  test("map", () => {
    expect(Chunk.of(1, 2, 3).map(x => x * 2).toArray()).toEqual([2, 4, 6])
  })

  test("filter", () => {
    expect(Chunk.of(1, 2, 3, 4).filter(x => x % 2 === 0).toArray()).toEqual([2, 4])
  })
...
describe("Stream constructors", () => {
  ...
  test("iterate", async () => {
    expect(await run(Stream.iterate(1, x => x * 2).take(5).runCollect()))
      .toEqual([1, 2, 4, 8, 16])
  })
  ...
  test("repeatValue + take", async () => {
    expect(await run(Stream.repeatValue("x").take(3).runCollect()))
      .toEqual(["x", "x", "x"])
  })
  ...
  test("bracket — resource released", async () => {
    ...
    const s = Stream.bracket(
      sync(() => { log.push("acquire"); return "conn" }),
      (r) => sync(() => { log.push(`release:${r}`) }),
    )
...
describe("Stream transforms", () => {
  test("map", async () => {
    expect(await run(Stream.of(1, 2, 3).map(x => x * 10).runCollect()))
      .toEqual([10, 20, 30])
  })

  test("filter", async () => {
    expect(await run(Stream.of(1, 2, 3, 4, 5).filter(x => x % 2 === 0).runCollect()))
      .toEqual([2, 4])
  })
  ...
  test("take", async () => {
    expect(await run(Stream.of(1, 2, 3, 4, 5).take(3).runCollect()))
      .toEqual([1, 2, 3])
  })

  test("drop", async () => {
    expect(await run(Stream.of(1, 2, 3, 4, 5).drop(2).runCollect()))
      .toEqual([3, 4, 5])
  })

  test("takeWhile", async () => {
    expect(await run(Stream.of(1, 2, 3, 4, 5).takeWhile(x => x < 4).runCollect()))
      .toEqual([1, 2, 3])
  })

  test("scan", async () => {
    expect(await run(Stream.of(1, 2, 3).scan(0, (a, b) => a + b).runCollect()))
      .toEqual([0, 1, 3, 6])
  })

  test("zipWithIndex", async () => {
    expect(await run(Stream.of("a", "b", "c").zipWithIndex().runCollect()))
      .toEqual([["a", 0], ["b", 1], ["c", 2]])
  })

  test("changes (deduplicate consecutive)", async () => {
    expect(await run(Stream.of(1, 1, 2, 2, 2, 3, 1, 1).changes().runCollect()))
      .toEqual([1, 2, 3, 1])
  })
  ...
  test("tap", async () => {
    const log: number[] = []
    await run(Stream.of(1, 2, 3).tap(x => log.push(x)).runDrain())
    expect(log).toEqual([1, 2, 3])
  })

  test("grouped", async () => {
    const result = await run(
      Stream.of(1, 2, 3, 4, 5).grouped(2).map(c => c.toArray()).runCollect()
    )
...
describe("Stream combination", () => {
  ...
  test("zip", async () => {
    ...
    expect(await run(s.runCollect())).toEqual([[1, "a"], [2, "b"], [3, "c"]])
  })

  test("zip stops at shorter", async () => {
    const s = Stream.of(1, 2, 3, 4).zip(Stream.of("a", "b"))
    expect(await run(s.runCollect())).toEqual([[1, "a"], [2, "b"]])
  })
...
describe("Stream error handling", () => {
  test("catch recovers", async () => {
    const s = Stream.of(1, 2).concat(Stream.fail("oops")).catch(() => Stream.of(99))
...
describe("Stream terminals", () => {
  ...
  test("runForEach", async () => {
    const log: number[] = []
    await run(Stream.of(1, 2, 3).runForEach(x => sync(() => { log.push(x) })))
    expect(log).toEqual([1, 2, 3])
  })
  ...
  test("runDrain", async () => {
    let count = 0
    await run(Stream.of(1, 2, 3).tap(() => count++).runDrain())
    expect(count).toBe(3)
  })
})
...
describe("Stream + services", () => {
  test("evalMap with service", async () => {
    ...
    interface Db { query(id: number): ReturnType<typeof succeed<string>> }
    ...
    const result = await run(
      provide(program, Db, { query: (id) => succeed(`row-${id}`) })
    )
