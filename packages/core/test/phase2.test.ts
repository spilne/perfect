...
import {
  succeed, fail, sync, suspend, sleep,
  fork, join, all, ensuring,
  run, runSync,
  Deferred, Queue, Semaphore, Ref,
  Schedule, retryWith, repeat,
} from "../src"
...
describe("Deferred", () => {
  ...
  test("await then succeed", async () => {
    const program = Deferred.make<number>().flatMap(d =>
      fork(sleep(10).flatMap(() => d.succeed(99))).flatMap(() =>
        d.await
      )
    )
  ...
  test("multiple awaiters", async () => {
    const program = Deferred.make<string>().flatMap(d =>
      all([d.await, d.await, d.await])
        .parZip(
          sleep(10).flatMap(() => d.succeed("hello")).as(undefined)
        )
        .map(([results]) => results)
    )
  ...
  test("succeed twice returns false", async () => {
    const program = Deferred.make<number>().flatMap(d =>
      d.succeed(1).flatMap(first =>
        d.succeed(2).map(second => [first, second])
      )
    )
  ...
  test("isDone", async () => {
    const program = Deferred.make<number>().flatMap(d =>
      d.isDone.flatMap(before =>
        d.succeed(1).flatMap(() =>
          d.isDone.map(after => [before, after])
        )
      )
    )
...
describe("Queue", () => {
  ...
  test("take blocks until offer", async () => {
    const program = Queue.unbounded<string>().flatMap(q =>
      fork(sleep(20).flatMap(() => q.offer("delayed")))
        .flatMap(() => q.take())
    )
    ...
    const program = Semaphore.make(1).flatMap(sem =>
      Ref.make(0).flatMap(maxConcurrent =>
        Ref.make(0).flatMap(current => {
          ...
          return all(Array.from({ length: 5 }, () => task))
            .flatMap(() => maxConcurrent.get)
        })
      )
    )
    ...
    const program = Semaphore.make(3).flatMap(sem =>
      Ref.make(0).flatMap(maxConcurrent =>
        Ref.make(0).flatMap(current => {
          ...
          return all(Array.from({ length: 10 }, () => task))
            .flatMap(() => maxConcurrent.get)
        })
      )
    )
      ...
      Semaphore.make(2).flatMap(sem =>
        Ref.make<number[]>([]).flatMap(results => {
          ...
          const consumer = (function consume(): any {
            return q.take()
              .flatMap(n => sem.withPermit(
                results.update(r => [...r, n])
              ))
  ...
  test("Deferred as coordination", async () => {
    const program = Deferred.make<void>().flatMap(gate =>
      Ref.make("waiting").flatMap(state => {
        const worker = gate.await.flatMap(() =>
          state.set("started")
        )
        return fork(worker)
          .flatMap(() => sleep(20))
          .flatMap(() => state.get)
          .flatMap(before =>
            gate.succeed(undefined).flatMap(() =>
              sleep(20).flatMap(() =>
                state.get.map(after => [before, after])
              )
            )
          )
      })
