...
describe("ref", () => {
  test("make, get, set", () => {
    const program = Ref.make(0).flatMap((ref) => ref.set(42).flatMap(() => ref.get));
    expect(runSync(program)).toBe(42);
  });

  test("update", () => {
    const program = Ref.make(10).flatMap((ref) => ref.update((x) => x + 5).flatMap(() => ref.get));
    expect(runSync(program)).toBe(15);
  });

  test("modify", () => {
    const program = Ref.make(10).flatMap((ref) =>
      ref.modify((x) => [`was:${x}`, x + 1]).zip(ref.get),
    );
    expect(runSync(program)).toEqual(["was:10", 11]);
  });

  test("getAndSet", () => {
    const program = Ref.make("old").flatMap((ref) => ref.getAndSet("new").zip(ref.get));
...
describe("parallel syntax", () => {
  ...
  test("parFlatMap", async () => {
    ...
    const result = await run(a.parFlatMap(b, (x, y) => succeed(x + y)));
...
describe("complex compositions", () => {
  ...
  test("service + retry + ensuring", async () => {
    ...
    const provided = provide(program, ApiClient, {
      fetch: (_url) =>
        sync(() => ++attempts).flatMap((n: number) =>
          n < 2 ? fail("fail") : succeed("response"),
        ) as any,
    });
  ...
  test("fork + ref + all", async () => {
    const program = Ref.make<string[]>([]).flatMap((log) => {
      const worker = (name: string) =>
        sleep(Math.random() * 10).flatMap(() => log.update((l) => [...l, name]));

      return all([worker("a"), worker("b"), worker("c")]).flatMap(() => log.get);
    });
