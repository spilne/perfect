...
describe("Stream fusion — fusible operators", () => {
  test("map transforms each item", async () => {
    const result = await run(Stream.of(1, 2, 3).map((x) => x * 2).runCollect())
    expect(result).toEqual([2, 4, 6])
  })

  test("chained maps compose in order", async () => {
    const result = await run(
      Stream.of(1, 2, 3).map((x) => x + 1).map((x) => x * 10).runCollect(),
    )
  ...
  test("filter keeps matching items", async () => {
    const result = await run(
      Stream.of(1, 2, 3, 4, 5).filter((x) => x > 3).runCollect(),
    )
  ...
  test("tap executes BEFORE map", async () => {
    const seen: number[] = []
    await run(
      Stream.of(10, 20, 30).tap((x) => seen.push(x)).map((x) => x + 1).runCollect(),
    )
...
describe("Stream fusion — terminals flush the buffer", () => {
  ...
  test("runHead / runLast", async () => {
    const head = await run(Stream.of(1, 2, 3).map((x) => x * 10).runHead())
    const last = await run(Stream.of(1, 2, 3).map((x) => x * 10).runLast())
    expect(head).toBe(10)
    expect(last).toBe(30)
  })
})
...
describe("Stream fusion — correctness vs unfused", () => {
  test("fused result matches mapChunks reference", async () => {
    ...
    const ref = input.map((x) => x + 1).filter((x) => x % 3 === 0).map((x) => x * 2)
  ...
  test("empty result after filter", async () => {
    const result = await run(
      Stream.of(1, 2, 3).filter(() => false).runCollect(),
    )
  ...
  test("filter with 'drop' action inverts the predicate", async () => {
    const result = await run(
      Stream.of(1, 2, 3, 4, 5).filter((x) => x > 3, "drop").runCollect(),
    )
  ...
  test("unNone fuses with adjacent ops", async () => {
    const s: any = Stream.of(1, null, 2, undefined, 3)
    const result = await run(s.unNone().map((x: number) => x * 10).runCollect())
