...
class NetError   extends Error { readonly _tag = "NetError" as const }

describe("trapError", () => {
  test("converts matching defect into a typed failure", async () => {
    const defectProne = sync(() => { throw new ParseError("bad input") }) as any
...
describe("validate", () => {
  test("returns tuple of values when all succeed", async () => {
    const result = await run(
      validate([succeed(1), succeed("two"), succeed(true)]) as any,
    )
  ...
  test("collects ALL failures into a Cause.both tree", async () => {
    const program = (validate([
      ...
      fail("b"),
      fail("c"),
    ]) as any).catchAllCause((cause: any) => succeed(Cause.failures(cause)))
...
describe("hedged", () => {
  test("fastest replica wins", async () => {
    ...
    let attempts = 0
    const work = sync(() => { attempts++; return "work" }) as any
  ...
  test("replicas=1 is a passthrough", async () => {
    let count = 0
    const eff = sync(() => { count++; return 42 }) as any
...
describe("repeatUntilWithBackoff", () => {
  test("exponential delays work on the test clock", async () => {
    ...
    // attempt 2 → 20ms
    c.advance(20); await new Promise((r) => setTimeout(r, 0))
...
describe("Cats-named aliases", () => {
  ...
  test("redeem transforms both channels with plain values", async () => {
    const ok  = (succeed(5) as any).redeem((_e: unknown) => -1, (v: number) => v * 2)
    const err = (fail("boom") as any).redeem((_e: unknown) => -1, (v: number) => v * 2)
...
describe("retry enhancements", () => {
  ...
  test("defects (thrown errors) are NOT retried by default", async () => {
    let attempts = 0
    const eff = sync(() => { attempts++; throw new Error("unexpected") }) as any
  ...
  test("retryAllCause stops when shouldRetry returns false", async () => {
    let attempts = 0
    const eff = sync(() => { attempts++; throw new TypeError("never-retry") }) as any
    await expect(
      run(retryAllCause(eff, {
        shouldRetry: (c) =>
          Cause.defects(c).some((d) => !(d instanceof TypeError)),
        times: 5,
        delayMs: 0,
      }) as any),
  ...
  test("jitter keeps delays within ±50% of base (statistical)", async () => {
    ...
    const eff: any = sync(() => ++attempts)
      .flatMap((n: number) => (n < 3 ? fail("retry") : succeed("ok")))
    const result = await run(
