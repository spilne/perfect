...
export class RealConsole implements Console {
  log(msg: string): Eff<void, never> {
    return effSync(() => { globalThis.console.log(msg) })
  }
  warn(msg: string): Eff<void, never> {
    return effSync(() => { globalThis.console.warn(msg) })
  }
  error(msg: string): Eff<void, never> {
    return effSync(() => { globalThis.console.error(msg) })
  }
}
...
export class TestConsole implements Console {
  ...
  log(msg: string): Eff<void, never> {
    return effSync(() => { this._logs.push(msg) })
  }
  warn(msg: string): Eff<void, never> {
    return effSync(() => { this._warns.push(msg) })
  }
  error(msg: string): Eff<void, never> {
    return effSync(() => { this._errors.push(msg) })
  }

  /** All log() messages, oldest first. */
  logs():   readonly string[] { return this._logs }
  /** All warn() messages, oldest first. */
  warns():  readonly string[] { return this._warns }
  /** All error() messages, oldest first. */
  errors(): readonly string[] { return this._errors }
