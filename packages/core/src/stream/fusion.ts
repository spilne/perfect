...
export function compileFused(ops: FusibleOp[]): (value: any) => any {
  ...
  if (ops.length === 2) {
    ...
    if (o1._tag === "map" && o2._tag === "filter") {
      const f = o1.fn, p = o2.fn
      return (v: any) => { const m = f(v); return p(m) ? m : SKIP }
    }
    // filter → map
    if (o1._tag === "filter" && o2._tag === "map") {
      const p = o1.fn, f = o2.fn
      return (v: any) => (p(v) ? f(v) : SKIP)
    }
    // map → map
    if (o1._tag === "map" && o2._tag === "map") {
      const f = o1.fn, g = o2.fn
      return (v: any) => g(f(v))
    }
    // filter → filter
    if (o1._tag === "filter" && o2._tag === "filter") {
      const p = o1.fn, q = o2.fn
      return (v: any) => (p(v) && q(v) ? v : SKIP)
    }
    // general 2-op fallback (includes tap / filterMap)
    const op1 = compileOne(o1), op2 = compileOne(o2)
    return (v: any) => {
      const r1 = op1(v); if (r1 === SKIP) return SKIP
      return op2(r1)
    }
  }
  ...
  return (value: any) => {
    ...
    for (let i = 0; i < ops.length; i++) {
      const op = ops[i]!
      switch (op._tag) {
        case "map":       v = op.fn(v); break
        case "filter":    if (!op.fn(v)) return SKIP; break
        case "filterMap": { const r = op.fn(v); if (r === undefined) return SKIP; v = r; break }
        case "tap":       op.fn(v); break
      }
    }
    return v
  }
...
function compileOne(op: FusibleOp): (v: any) => any {
  switch (op._tag) {
    case "map":       return op.fn
    case "filter":    return (v: any) => (op.fn(v) ? v : SKIP)
    case "filterMap": return (v: any) => {
      const r = op.fn(v)
      return r === undefined ? SKIP : r
    }
    case "tap":       return (v: any) => { op.fn(v); return v }
  }
}
