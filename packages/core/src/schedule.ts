...
export const Schedule = {
  ...
  whileInput<In, Out>(schedule: Schedule<In, Out>, pred: (input: In) => boolean): Schedule<In, Out> {
    return {
      initial: schedule.initial,
      step: (input, state) => {
  ...
  whileOutput<In, Out>(schedule: Schedule<In, Out>, pred: (output: Out) => boolean): Schedule<In, Out> {
    return {
      initial: schedule.initial,
      step: (input, state) => {
        const decision = schedule.step(input, state)
...
export function retryWith<A, S, In>(
  ...
  toInput?: (error: unknown) => In,
): Eff<A, S> {
  function loop(state: any): Eff<A, S> {
    return (new Suspend(Op.CatchAll, eff, (cause: any) => {
      const input = toInput
        ? toInput(cause._tag === "Fail" ? cause.error : cause)
        : cause as any
      const decision = schedule.step(input, state)
      if (decision._tag === "Done") return new Suspend(Op.Fail, cause, null)
      if (decision.delay > 0) {
...
export function repeat<A, S>(
  eff: Eff<A, S>,
  schedule: Schedule<A, any>,
): Eff<A, S> {
  function loop(state: any): Eff<A, S> {
