...
function stringGen(opts: { length?: number; minLength?: number; maxLength?: number; alphabet?: string } = {}): Gen<string> {
      ...
      (r: Random) => {
        ...
        return new Suspend(Op.FlatMap, lenEff, (n: number) => {
          ...
          const collect = (i: number, acc: string): Eff<string, never> => {
            ...
            return new Suspend(
              Op.FlatMap,
              (r.nextInt(alphabet.length) as any),
              (idx: number) => collect(i + 1, acc + alphabet[idx]),
            ) as any
          }
          return collect(0, "")
        })
...
function choiceGen<A>(...items: A[]): Gen<A> {
  ...
  return mk(
    new Suspend(Op.FlatMap, Random.get as any, (r: Random) => r.choice(items)) as any,
  )
}

function arrayGen<A>(item: Gen<A>, opts: { length?: number; minLength?: number; maxLength?: number } = {}): Gen<A[]> {
        ...
        return new Suspend(Op.FlatMap, lenEff, (n: number) => {
          const collect = (i: number, acc: A[]): Eff<A[], never> => {
            ...
            return new Suspend(
              Op.FlatMap,
              item.generate,
              (a: A) => { acc.push(a); return collect(i + 1, acc) },
            ) as any
          }
          return collect(0, [])
        })
    ...
    (() => {
      const collect = (i: number, acc: any): Eff<any, never> => {
        ...
        return new Suspend(
          Op.FlatMap,
          shape[k]!.generate,
          (v: any) => { acc[k] = v; return collect(i + 1, acc) },
        ) as any
      }
      return collect(0, {})
    })(),
    ...
    (() => {
      const collect = (i: number, acc: any[]): Eff<any[], never> => {
        ...
        return new Suspend(
          Op.FlatMap,
          gens[i]!.generate,
          (v: any) => { acc.push(v); return collect(i + 1, acc) },
        ) as any
      }
      return collect(0, [])
    })(),
