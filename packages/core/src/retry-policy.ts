      ...
      if (decision._tag === "Done") {
        if (onRetry) {
          return ((onRetry({
            ...
            givingUp: true,
          }) as any).flatMap(() => new Suspend(Op.Fail, cause, null))) as any
        }
        return new Suspend(Op.Fail, cause, null)
      }

      const next = () => {
        const wait = decision.delay > 0 ? (sleep(decision.delay) as any) : (succeed(undefined) as any)
