...
function foldLeafs<E, B>(
  ...
  switch (c._tag) {
    case "Fail":      return onFail(c.error)
    case "Die":       return onDie(c.defect)
    case "Interrupt": return onInterrupt()
    case "Both":
    case "Then":      return combine(
...
export const Cause = {
  ...
  both: <E>(left: Cause<E>, right: Cause<E>): Cause<E> =>
    ({ _tag: "Both", left, right }),

  then: <E>(left: Cause<E>, right: Cause<E>): Cause<E> =>
    ({ _tag: "Then", left, right }),
  ...
  failures: <E>(c: Cause<E>): E[] => {
    const out: E[] = []
    foldLeafs<E, void>(c, (e) => { out.push(e) }, () => {}, () => {}, () => {})
    return out
  },

  defects: <E>(c: Cause<E>): unknown[] => {
    const out: unknown[] = []
    foldLeafs<E, void>(c, () => {}, (d) => { out.push(d) }, () => {}, () => {})
    return out
  },

  hasFail: <E>(c: Cause<E>): boolean =>
    foldLeafs<E, boolean>(c, () => true, () => false, () => false, (a, b) => a || b),

  hasDie: <E>(c: Cause<E>): boolean =>
    foldLeafs<E, boolean>(c, () => false, () => true, () => false, (a, b) => a || b),

  hasInterrupt: <E>(c: Cause<E>): boolean =>
    foldLeafs<E, boolean>(c, () => false, () => false, () => true, (a, b) => a || b),
  ...
  firstFail: <E>(c: Cause<E>): { value: E } | null => {
    switch (c._tag) {
      case "Fail":      return { value: c.error }
      case "Die":
      case "Interrupt": return null
      case "Both":
      case "Then":      return Cause.firstFail(c.left) ?? Cause.firstFail(c.right)
    }
  },
  ...
  firstDie: <E>(c: Cause<E>): { value: unknown } | null => {
    switch (c._tag) {
      case "Die":       return { value: c.defect }
      case "Fail":
      case "Interrupt": return null
      case "Both":
      case "Then":      return Cause.firstDie(c.left) ?? Cause.firstDie(c.right)
    }
  },
  ...
  squash: <E>(c: Cause<E>): unknown => {
    const f = Cause.firstFail(c); if (f) return f.value
    const d = Cause.firstDie(c);  if (d) return d.value
    return new Error("Interrupted")
  },

  pretty: <E>(c: Cause<E>): string => {
    switch (c._tag) {
      case "Fail":      return `Fail(${String(c.error)})`
      case "Die":       return `Die(${String(c.defect)})`
      case "Interrupt": return "Interrupt"
      case "Both":      return `(${Cause.pretty(c.left)} & ${Cause.pretty(c.right)})`
      case "Then":      return `(${Cause.pretty(c.left)} ; ${Cause.pretty(c.right)})`
    }
  },
} as const
