...
for (const count of [10, 100, 1_000]) {
  group(`all × ${count}`, () => {
    ...
    bench("Promise.all", async () =>
      Promise.all(items.map((i) => Promise.resolve(i * 2))),
    )

    bench("perfect all + run", async () =>
      pRun(pAll(items.map((i) => pSucceed(i * 2))) as any),
    )
...
group("race × 3 (all succeed fast)", () => {
  ...
  bench("perfect race × 3 + run", async () =>
    pRun(pRace([pSucceed(1), pSucceed(2), pSucceed(3)])),
  )

  bench("effect race × 3 + runPromise", async () =>
    Effect.runPromise(Effect.race(Effect.race(Effect.succeed(1), Effect.succeed(2)), Effect.succeed(3))),
  )
})
...
group("error: fail → handle → succeed", () => {
  ...
  bench("effect fail + catchAll + runPromise", async () =>
    Effect.runPromise(
      Effect.fail("boom").pipe(Effect.catchAll(() => Effect.succeed(0))),
    ),
  )
})
...
group("retry: succeeds first try", () => {
  bench("perfect retry × 3 + run", async () =>
    pRun(pRetry(pSucceed(42), { times: 3 })),
  )
...
group("sleep(0) chain × 50", () => {
  ...
  const buildEffect = (): Effect.Effect<number> => {
    let e: Effect.Effect<number> = Effect.succeed(0)
    for (let i = 0; i < N; i++) {
      e = e.pipe(Effect.flatMap((x) => Effect.sleep(0).pipe(Effect.flatMap(() => Effect.succeed(x + 1)))))
    }
    return e
  }
