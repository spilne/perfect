import {
  ...
  succeed, fail, sync, sleep, delay,
  tryPromise, ensuring, retry, timeout, all,
  fork, join,
  service, provide,
  run, Ref,
} from "../src"
...
interface User { id: string; name: string; email: string; tier: "free" | "pro" }
interface Order { id: string; userId: string; amount: number; status: string }
interface Invoice { orderId: string; amount: number; sent: boolean }

class NotFound   { readonly _tag = "NotFound"   as const; constructor(public entity: string, public id: string) {} }
class RateLimit  { readonly _tag = "RateLimit"  as const; constructor(public retryAfter: number) {} }
class Timeout    { readonly _tag = "Timeout"    as const }
...
const getUserOrders = (userId: string) =>
  Logger.get.flatMap(log =>
    log.info(`Fetching orders for ${userId}`).flatMap(() =>
      timeout(
        UserRepo.get.flatMap(repo => repo.find(userId))
          .flatMap(user =>
            OrderRepo.get.flatMap(orders => orders.findByUser(user.id))
...
const processUser = (userId: string) =>
  Logger.get.flatMap(log =>
    getUserOrders(userId)
        ...
        return all(orders.map(order => sendInvoice(user, order)))
          .flatMap(() =>
            ...
            all(orders.map(order =>
              OrderRepo.get.flatMap(repo =>
                repo.updateStatus(order.id, "invoiced")
              )
            ))
          )
      ...
      .catchTag("Timeout", () =>
        log.error(`Timed out fetching orders for ${userId}`).as(0)
      )
      .catchTag("NotFound", (e) =>
        log.error(`${e.entity} not found: ${e.id}`).as(0)
      )
  )
...
const processAllUsers = (userIds: string[]) =>
  Ref.make(0).flatMap(counter =>
    all(
      userIds.map(id =>
        processUser(id).flatMap(count =>
          counter.update(n => n + count)
        )
      )
    )
    .flatMap(() => counter.get)
    .flatMap(total =>
      Logger.get.flatMap(log =>
        log.info(`Done. Processed ${total} orders total.`).as(total)
      )
    )
  )
...
const program = withConnectionLogging(
  processAllUsers(["user-1", "user-2", "user-404"])
)
    ...
    provide(
      provide(
        provide(program,
          Logger,
          {
            info:  (msg) => sync(() => { logs.push(`[INFO]  ${msg}`) }),
            error: (msg) => sync(() => { logs.push(`[ERROR] ${msg}`) }),
          },
        ),
        UserRepo,
        {
      ...
      {
        findByUser: (userId) =>
          succeed(fakeOrders.filter(o => o.userId === userId)),
