...
group(`chained map × 5 over ${N}`, () => {
  bench("Array chain", () =>
    items.map(x => x + 1).map(x => x * 2).map(x => x - 1).map(x => x / 3 | 0).map(x => x + 100),
  )
...
async function asyncIterMap(src: number[]): Promise<number[]> {
  const out: number[] = []
  async function* gen(): AsyncGenerator<number> { for (const x of src) yield x }
...
group(`async iterator vs stream (${N} items)`, () => {
  ...
  bench("perfect Stream.map",    async () => pRun(PStream.fromArray(items).map((x) => x * 2).runCollect()))

  bench("async-iter map+filter", async () => asyncIterMapFilter(items))
  bench("perfect map+filter",    async () =>
    pRun(PStream.fromArray(items).map((x) => x * 2).filter((x) => x % 3 === 0).runCollect()),
  )
  ...
  bench("perfect Stream.runFold",async () => pRun(PStream.fromArray(items).runFold(0, (a, b) => a + b)))
})
...
function slowWork(_: number): number {
  ...
  while (Bun.nanoseconds() - t < SLOW_US * 1000) { /* spin */ }
  return _
}
...
group(`slow consumer × ${BP_N} (${SLOW_US}µs/item)`, () => {
  ...
  bench("async-iter slow consumer", async () => {
    async function* gen() { for (const x of src) yield x }
    let sum = 0
  ...
  bench("perfect Stream slow consumer", async () =>
    pRun(PStream.fromArray(src).map(slowWork).runFold(0, (a, b) => a + b)),
  )

  bench("RxJS slow consumer", async () =>
    lastValueFrom(rxFrom(src).pipe(rxMap(slowWork), rxScan((a: number, b: number) => a + b, 0))),
  )
})
