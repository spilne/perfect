...
const getUser = (id: string) =>
  Logger.get.flatMap((log) =>
    log.info(`looking up user ${id}`).flatMap(() => UserRepo.get.flatMap((repo) => repo.find(id))),
  );
...
const getAllNames = all([getUserName("1"), getUserName("2"), getUserName("999")]);
// type: Eff<string[], Throws<DbError> | Needs<Logger> | Needs<UserRepo>>
