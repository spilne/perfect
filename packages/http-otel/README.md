...
Drop-in OpenTelemetry tracing for `@perfect/http`. Two integration points
...
This package is HTTP-specific. For the general bridge — running `@perfect/core`'s
`Tracer` service and `MetricsRegistry` on OpenTelemetry — use `@perfect/otel`.
...
The default tracer is `trace.getTracer("@perfect/http")`; pass your own via
...
- General core Tracer/Metrics bridge: `@perfect/otel`
